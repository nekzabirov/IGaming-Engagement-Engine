...
- `IJourneyNode` is an abstract class (not interface) with `prev`/`next` as constructor parameters and built-in circular dependency validation in `init`
- `ITriggerJourneyNode` is an abstract class extending `IJourneyNode` — subclass data classes pass `prev`/`next` as private constructor params forwarded to super
