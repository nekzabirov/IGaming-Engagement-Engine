...
- Condition journey nodes: `<Name>ConditionJourneyNode` extending `IConditionJourneyNode` (abstract class with `inputKey`, `matchNode`, `notMatchNode`), in `infrastructure/journey/condition/` package. Concrete subtypes grouped as sealed classes (e.g., `NumberConditionJourneyNode`). Process: `ConditionJourneyNodeProcess` (handles all subtypes). Each concrete condition type has its own nomenclature: `<Name>ConditionNomenclature` (e.g., `BoolConditionNomenclature`, `NumberInRangeConditionNomenclature`, `StringEqualConditionNomenclature`) with empty input/output params
